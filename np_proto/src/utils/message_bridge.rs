...
pub fn proxy_message_2_pb(proxy_message: ProxyMessage, tunnel_id: u32) -> MessageType {
    match proxy_message {
        ProxyMessage::I2oConnect(session_id, is_tcp, is_compressed, addr, encryption_method, encryption_key, client_addr) => MessageType::GenericI2oConnect(generic::I2oConnect {
            tunnel_id,
            session_id,
            is_tcp,
            addr,
...
pub fn is_i2o_message(proxy_message: &ProxyMessage) -> bool {
    match proxy_message {
        ProxyMessage::I2oConnect(_, ..) | ProxyMessage::I2oSendData(_, ..) | ProxyMessage::I2oDisconnect(_) | ProxyMessage::I2oRecvDataResult(_, ..) => true,

        ProxyMessage::O2iConnect(_, ..) | ProxyMessage::O2iSendDataResult(_, ..) | ProxyMessage::O2iRecvData(_, ..) | ProxyMessage::O2iDisconnect(_, ..) => false,
    }
}
