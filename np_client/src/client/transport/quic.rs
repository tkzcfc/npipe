...
impl QuicTransportState {
    pub async fn configure_from_login(
        ...
        *guard = token;
        info!(
            "QUIC transport pool configured, max_forward_streams:{}, idle_timeout_secs:{}",
    ...
    async fn get_or_create_forward_path(
        &self,
        session_id: u32,
    ) -> anyhow::Result<Option<Arc<QuicForwardPath>>> {
        let max_paths = self.max_forward_paths.load(Ordering::Relaxed);
        if max_paths == 0 {
            debug!(
                "QUIC transport pool disabled, proxy_session_id:{session_id} uses control stream"
            );
            return Ok(None);
        }
        ...
        if token.is_empty() {
            debug!("QUIC transport token empty, proxy_session_id:{session_id} uses control stream");
            return Ok(None);
        }
        ...
        let path = if self.forward_paths.len() < max_paths as usize {
            ...
            debug!(
                "QUIC transport pool reached limit, proxy_session_id:{}, current_streams:{}, max_streams:{}",
                session_id,
                self.forward_paths.len(),
                max_paths
            );
            self.select_least_loaded_path()
                .ok_or_else(|| anyhow!("no available QUIC forward path"))?
        };
    ...
    async fn open_forward_path(&self, token: String) -> anyhow::Result<Arc<QuicForwardPath>> {
        let connection_id = self.next_connection_id.fetch_add(1, Ordering::Relaxed);
        info!("opening QUIC forward stream, connection_id:{connection_id}");
        let stream = {
            let mut handle = self.handle.lock().await;
            handle.open_bidirectional_stream().await?
        };
        let (reader, writer) = tokio::io::split(stream);
        let writer = Arc::new(Mutex::new(writer));
        let path = Arc::new(QuicForwardPath {
            connection_id,
            writer: writer.clone(),
        ...
        debug!(
            "QUIC forward stream opened, connection_id:{}, current_streams:{}",
            connection_id,
            self.forward_paths.len()
        );
    ...
    fn select_least_loaded_path(&self) -> Option<Arc<QuicForwardPath>> {
        self.forward_paths
            .iter()
    ...
    pub async fn remove_forward_path(&self, connection_id: u64) {
        ...
        if let Some((_, path)) = self.forward_paths.remove(&connection_id) {
            info!(
                "remove QUIC forward stream, connection_id:{}, remaining_streams:{}",
                connection_id,
                self.forward_paths.len()
            );
    ...
    async fn close_idle_forward_paths(&self) {
        ...
        for connection_id in connection_ids {
            if let Some((_, path)) = self.forward_paths.remove(&connection_id) {
                info!(
                    "close idle QUIC forward stream, connection_id:{}, remaining_streams:{}",
                    connection_id,
                    self.forward_paths.len()
                );
                let _ = path.writer.lock().await.shutdown().await;
